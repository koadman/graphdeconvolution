...
void UnitigGraph::InitFromSdBG() {
    ...
    // adjust the depth calculations
/*    for (vertexID_t i = 0; i < vertices_.size(); ++i) {
        int sdepth = sdbg_->EdgeMultiplicity(vertices_[i].start_node);
        int edepth = sdbg_->EdgeMultiplicity(vertices_[i].end_node);
        vertices_[i].depth -= (sdepth+edepth);
    }
*/
...
void UnitigGraph::ComputeSampleDepths(vector<SuccinctDBG>& sample_sdbg, const std::string& output_prefix) {
    uint8_t ltab[256];
    ltab['A']=1;
    ltab['C']=2;
    ltab['G']=3;
    ltab['T']=4;
    vector< std::pair< int64_t, int64_t > > adj_list[4];
...
//        std::cerr << "Node " << i << std::endl;
        ...
        for (int strand = 0; strand < 2; ++strand) {
            ...
            // count up number of outgoings to active nodes
            int active_out = 0;
            for (int j = 0; j < outdegree; ++j) {
                auto next_vertex_iter = start_node_map_.find(outgoings[j]);
                auto v = vertices_[next_vertex_iter->second];
                if( !v.is_dead && !v.is_deleted ) active_out++;
            }

            // store outgoings to active nodes
            for (int j = 0; j < outdegree; ++j) {
                auto next_vertex_iter = start_node_map_.find(outgoings[j]);
                auto v = vertices_[next_vertex_iter->second];
                if( v.is_dead || v.is_deleted ) continue;
                int64_t fmult = strand == 0 ? 1 : -1;
                int64_t rmult = v.start_node == outgoings[j] ? 1 : -1;
                adj_list[active_out-1].push_back(std::make_pair(fmult*((int64_t)i+1), rmult * ((int64_t)next_vertex_iter->second+1)));
            }
        }
    }

    std::string depth_out_fname = output_prefix + ".unitig_depths.Rdata";
    ...
    sep = '(';
    std::ostringstream adj1sstr;
    std::ostringstream adj1dstr;
    adj1sstr << "adj1source<-c";
    adj1dstr << "adj1dest<-c";
    for (size_t i=0; i<adj_list[0].size(); i++){
        adj1sstr << sep << adj_list[0][i].first;
        adj1dstr << sep << adj_list[0][i].second;
        sep = ',';
    }
    depth_out << "adj1count<-" << adj_list[0].size() << "\n";
    depth_out << adj1sstr.str() << ")\n" << adj1dstr.str() << ")\n";

    sep = '(';
    std::ostringstream adj2sstr;
    std::ostringstream adj2d1str;
    std::ostringstream adj2d2str;
    adj2sstr << "adj2source<-c";
    adj2d1str << "adj2dest1<-c";
    adj2d2str << "adj2dest2<-c";
    int used = 0;
    for (size_t i=0; i<adj_list[1].size(); i+=2){
        adj2sstr << sep << adj_list[1][i].first;
        adj2d1str << sep << adj_list[1][i].second;
        adj2d2str << sep << adj_list[1][i+1].second;
        sep = ',';
        used++;
    }
    depth_out << "adj2count<-" << used << "\n";
    depth_out << adj2sstr.str() << ")\n" << adj2d1str.str() << ")\n" << adj2d2str.str() << ")\n";

    // write out vertex lengths
    depth_out << "lengths<-c";
    sep = '(';
    for (vertexID_t i = 0; i < vertices_.size(); ++i) {
        depth_out << sep << vertices_[i].length;
        sep = ',';
    }
    depth_out << ")\n";
    ...
    std::string unitig_out_fname = output_prefix + ".unitigs.fa";
    FILE *unitig_file = OpenFileAndCheck(unitig_out_fname.c_str(), "w");
    long long output_id = 1;
    ...
    for (vertexID_t i = 0; i < vertices_.size(); ++i) {
        ...
        UnitigGraphVertex &vertex = vertices_[i];
        string label = VertexToDNAString(sdbg_, vertex);
        WriteContig(label, sdbg_->kmer_k, output_id, 0, vertex.depth * 1.0 / vertex.length, &output_lock, unitig_file);
    }    
}

uint32_t UnitigGraph::MergeBubbles(bool permanent_rm, bool careful, FILE *bubble_file, Histgram<int64_t> &hist, float bubble_pop_depth_ratio_threshold) {
    ...
    for (vertexID_t i = 0; i < vertices_.size(); ++i) {
        ...
        for (int strand = 0; strand < 2; ++strand) {
            ...
            // AD: do not pop if there is support for this bubble
//            if( outdegree == 2 && -std::get<0>(branches[1]) >= -std::get<0>(branches[0]) * bubble_pop_depth_ratio_threshold )
//                continue;
