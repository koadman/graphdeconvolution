...
int main_assemble(int argc, char **argv) {
    ...
    if (opt.factorize>0) { // factorize the graph into strain components
        // pop the very poorly supported bubbles before writing the unitig graph
/*        Histgram<int64_t> bubble_hist;
        uint32_t num_bubbles = unitig_graph.MergeBubbles(true, opt.careful_bubble, NULL, bubble_hist, 0.3);
        std::cerr << "Popped " << num_bubbles << " poorly supported bubbles\n";
*/
        ...
        unitig_graph.ComputeSampleDepths(dbg_s,opt.output_prefix);
    }
